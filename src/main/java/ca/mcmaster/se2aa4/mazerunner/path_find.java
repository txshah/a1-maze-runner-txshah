...
//need to confirm that tag is -i 
  /*   
    ...
    public path_find(String MAZE_FILE) throws IOException{   
        ...
        int loopCount = 0;
        System.out.println("Path_find function");
        
        System.out.println("currentpoint: " + current_point[0]+"," + current_point[1]);
        System.out.println("endPoint: " + end_point[0] + ","+end_point[1]);
        
        while(!(Arrays.equals(current_point,end_point))){
    ...
    public void right(){
        ...
        current_point[0] = row+1;//current point = updated 
        row = row + 1;//row updated 
        output += "RF";//string output updated 
        prev = "right";
        System.out.println("RF");
        System.out.println(col + row + current_point[0]+ current_point[1]);

    }
    public void forward(){
        //goes forward ("east direction") - need to add F)
        current_point[1] = col +1;//current point = updated 
        col = col+1;//x updated 
